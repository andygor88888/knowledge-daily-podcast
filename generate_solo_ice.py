...
SCRIPT_FILE = r"D:\\hermes\\podcast\\script_solo_ice.txt"
OUTPUT_DIR = r"D:\\hermes\\podcast\\solo_segments"
...
current = ""
for p in paragraphs:
    if len(current) + len(p) < 300:  # Keep chunks under ~300 chars for good TTS
        current += p + "\n\n"
    else:
        if current:
...
def call_tts(text, chunk_idx):
    payload = {
        "model": "mimo-v2.5-tts",
        "messages": [
            {"role": "user", "content": "自然知性女聲，像知識型 podcast 主持人，咬字清晰，語氣真誠溫柔，語速適中"},
