...
def process_script(script_file, output_dir, label):
    ...
    for i, chunk_text in enumerate(chunks):
        ...
        if os.path.exists(output_file) and os.path.getsize(output_file) > 1000:
            print(f"  ⏭️ [{label}] [{i}/{len(chunks)}] exists")
            continue
...
# Process all 4 scripts
process_script("script_gpt56_zh.txt", "gpt56_seg", "GPT-5.6")
process_script("script_apple_openai_zh.txt", "apple_seg", "Apple")
process_script("script_bone_sugar_zh.txt", "bone_seg", "骨骼-血糖")
process_script("script_root_canal_zh.txt", "root_seg", "根管治療")
